...
def accumulative_train(trainset, hp, model):
    """Load an existing model then train and evaluate it
...
def my_inference_on_tables(tables,
    ...
    cols=[]
    timess=[]
    for tid, name in tqdm(enumerate(tables), total=total):
        now = time.time()
        x, _ = unlabeled._tokenize(tables[name])
        names.append(name)
        rows.append(tables[name].shape[0])
        cols.append(tables[name].shape[1])
        ...
        if tid == total - 1 or len(batch) == batch_size:
                ...
                for xi in x:
                    ...
                    for token_id in xi:
                        if token_id == unlabeled.tokenizer.cls_token_id:
                            ##to notmalize
                            #temp_vec = column_vectors[ptr].cpu().numpy()
                            ## normalizing the vectors
                            #norm = np.linalg.norm(temp_vec)
                            #norm_temp_vec= temp_vec / norm 
                            #current.append(norm_temp_vec)
            ...
            timess.append(time.time()-now)
    return results,names,rows,cols,timess
...
def load_checkpoint(ckpt):
    ...
    if hp.task == "santosLarge":
        ...
        ds_path = 'data/tus/small/benchmark'
    elif hp.task == "tusLarge":
        ds_path = 'data/tus/large/benchmark'
    elif hp.task == "wdc":
        ds_path = 'data/wdc/0'
